...
#[test]
fn system_with_native_returns_faces_on_macos() {
    let catalog = oxifont::system_with_native()
        .expect("system_with_native() must succeed on macOS with CoreText");
    use oxifont_core::FontCatalog as _;
    assert!(
        !catalog.faces().is_empty(),
        "CoreText must enumerate at least one system font"
    );
    // Verify the first face has a non-empty family name.
    let first = &catalog.faces()[0];
    assert!(
        !first.family.is_empty(),
        "face must have a non-empty family"
    );
    assert!(
        first.path.exists(),
        "face path must point to an existing file"
    );
}

#[cfg(all(feature = "native", target_os = "macos"))]
#[test]
fn native_catalog_load_face_bridge() {
    use oxifont_core::FontCatalog as _;
    let catalog = oxifont::system_with_native().expect("CoreText enumeration must succeed");
    // Pick the first face and load it through the parser bridge.
    let info = &catalog.faces()[0];
    let face = catalog
        .load_face(info)
        .expect("load_face must parse a system font without error");
    use oxifont::FontFace as _;
    assert!(
        face.glyph_count() > 0,
        "parsed face must have at least one glyph"
    );
}
